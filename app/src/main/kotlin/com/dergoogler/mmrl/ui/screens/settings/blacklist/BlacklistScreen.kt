...
fun BlacklistScreen(
    ...
    val event =
        requestString(url = "https://raw.githubusercontent.com/wiki/DerGoogler/MMRL/Blacklisted-Modules.md",
            onSuccess = { blacklist = it })
    ...
    Scaffold(
        ...
        Column(
            ...
            AnimatedVisibility(
                visible = event.isLoading, enter = fadeIn(), exit = fadeOut()
            ) {
                Loading(minHeight = 200.dp)
            }

            AnimatedVisibility(
                visible = event.isSucceeded, enter = fadeIn(), exit = fadeOut()
            ) {
                MarkdownText(
                    text = blacklist,
